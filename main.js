...
// @match   https://*.waze.com/editor/editor*
// @match   https://*.waze.com/*/editor*
...
// **** Ajout Fonctions nouvel onglet
            ...
            document.getElementById ('WME_CRN_ShowChangeAlt').onclick = function() {showChangeAlt1()}
...
// ****Boite de dialogue CRN
            ...
            var btn8 = 	$('<button class="waze-btn waze-btn-small waze-btn-white" style="padding:0px 6px; height:20px; margin:5px" >Descartar </button>');
			var btn9 = 	$('<button class="waze-btn waze-btn-small waze-btn-white" style="padding:0px 6px; height:20px; margin:5px" >Alterar</button>');
...
// *** button d'appel de change alternate dans le panneau d'edition
            var btn4 = $('<button class="waze-btn waze-btn-small waze-btn-white" id="WME_CRN_ChangeToAltern1" style="display:none ; padding:0px 6px; height:20px; margin: 2px 10%;width: 80%";" title="Click this button to change to alternate name">Change to Altern. Name (CRN)</button>'); btn4.click(ChAlternate);
            $ ("#edit-panel").before(btn4);
 ...
 //                   add_alternative_street_city (lieuDit,cityName);										// on ajoute le lieudit en alternate de la ville
   ...
   function showChangeAlt1(ev) {
       if (document.getElementById ('WME_CRN_ShowChangeAlt').checked ==1){
           document.getElementById("WME_CRN_ChangeToAltern1").style.display = "block";}
        else {
            document.getElementById("WME_CRN_ChangeToAltern1").style.display = "none";}
       saveLocalStorage();
    }
...
//        if (findPending().length=== 0  && W.map.getOLMap().zoom > 1 && limitForSaveNotReach() )  {  // wait for loading
		if (W.map.getOLMap().zoom > 1 && limitForSaveNotReach() && document.getElementById("WME_CRN_DialogBox").style.display != "block" ) { // on ne relance pas s'il y a une modif en cours
            ...
            if (!limitForSaveNotReach()){myAlert ("<FONT color='red'><b>Por favor, salve e tente novamente</b></FONT>");}
                ...
                if (newName!=name && limitForSaveNotReach()){
                    ...
                    if (mySegs.main.length + mySegs.alternate.length + myLdmkToCheckStreetID.length > 0) {
                        ...
                        if (mySegs.main.length>0) {
                            createCheckBox ("WME_CRN_DialogBoxMain",mySegs.main.length+" rua(s) para alterar no endereço principal");
                        }
                        if (mySegs.alternate.length>0) {
                            createCheckBox ("WME_CRN_DialogBoxAlternate",mySegs.alternate.length+" rua(s) para alterar no endereço alternativo");
                        }
                        if (myLdmkToCheckStreetID.length>0) {
                            createCheckBox ("WME_CRN_DialogBoxLandmark",myLdmkToCheckStreetID.length+" local(is) para alterar o endereço");
                        }
                        var name1 = split_diff(name,newName);
                        var message1 = "\nCidade: "+city.attributes.name+"<br>"
                        message1 += "Estado: "+ W.model.states.objects[city.attributes.stateID].name+"<br>"
                        message1 += "Nome anterior é: "+name1.debut+"<span style='background-color:pink;color: red'>"+name1.milieu+"</span>"+name1.fin+"<br><br>"
                        message1 += "Confirme o novo nome ou altere-o";
...
// on traite les noms des landmark
		    if (checkLayerState ("venues")) {
                ...
                for (ldmkID in myWazeVenues) {
                    ...
                    if (ldmk!== null && ldmk.state !== "Apagar" && onScreen(ldmk) && (ldmk.attributes.approved) &&  (ldmk.attributes.streetID!== null) && ldmk.isAllowed(ldmk.permissionFlags.EDIT_PROPERTIES) && !ldmk.attributes.adLocked) {
            ...
            if (ldmark!==null && onScreen(ldmark) && ldmkIsAllowed(ldmark)) {
                ...
                if (oldName !==null && oldName !=="" && !isInArray(oldName,WME_CRN_badStreet)) {
                    ...
                    if (newName !=oldName && limitForSaveNotReach() ) {
                        ...
                        var message1 = "\nTipo : " + ldmark.attributes.categories +"<br><br>";
                        message1 += "\n\nEstado : " + state.name +"<br>";
                        message1 += "\nCidade : " + cityName +"<br>";
                        message1 += "Nome anterior : "+name1.debut+"<span style='background-color:pink;color: red'>"+name1.milieu+"</span>"+name1.fin+"<br><br>"
                        message1 += "\n\nConfirme o novo nome ou altere" ;

						document.getElementById("WME_CRN_DialogBoxTitle").innerHTML = message1;
                        document.getElementById("WME_CRN_newName").value = newName;
                        var myJson = {name: oldName,mySegs: {main: [], alternate: []}, myLdmkToCheckStreetID : [listLmrkIDs[i]],modPolygonIds:[]};
                        document.getElementById("WME_CRN_myJson").innerHTML = JSON.stringify(myJson);
                        createCheckBox ("WME_CRN_DialogBoxLandmark"," 1 local para renomear");
						document.getElementById("WME_CRN_DialogBox").style.display="block"; // on affiche la boite de dialogue
                    }
                }
            }
...
//            if (layers[i].options.uniqueName && layers[i].options.uniqueName.toUpperCase() == layerName.toUpperCase()) {index=i;}
    ...
    function manage_WME_CRN(ev) {
        if(document.getElementById('WME_CRN_enable').checked == 1) {
            ...
            if (userIsFrenchCM() && isInArray(WME_CRN_MainCountry,WME_CRN_country.frenchCountry)) {
                ...
                document.getElementById ('WME_CRN_ShowChangeAlt').checked ==1
            }
            showChangeAlt1();
        }
        else {
    ...
    function traiteSelection () {
        if (document.getElementById ('WME_CRN_StreetID').checked ==1){afficheSegProperties(); return; }
        var selectedItems = W.selectionManager.getSelectedFeatures();
    ...
    function restoreLocalStorage() {
        if(localStorage.CRN){
            ...
            getId('WME_CRN_ShowChangeAlt').checked = CRN_config.WME_CRN_ShowChangeAlt;
            manage_WME_CRN ();
        }
    }
    ...
    function saveLocalStorage() {
        ...
        CRN_config.WME_CRN_ShowChangeAlt = getId('WME_CRN_ShowChangeAlt').checked;
